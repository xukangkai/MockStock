...
系统已升级为基于 LangGraph 的多节点 Agent 决策架构：

```
记忆召回 → 市场分析 → 持仓审视 → 候选研究 → 风控审查 → 决策综合
```

- **6 个专家节点**串联执行，每步输出传递给下一步
- **安全回退**：Agent 失败时自动降级为传统单次 AI 决策
- **记忆积累**：每次决策写入记忆库，日终自动总结
- **可视化**：前端 🧠 Agent 决策链 tab 查看决策时间线和记忆
